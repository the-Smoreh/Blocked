// Repoints book URLs from one host to another, in bulk.
//
// Why this exists: 440 of the books in this library share a single host,
// `mathematics-lessons.eclipsecastellon.com`. That subdomain is NXDOMAIN as of
// 2026-09-15, which is why those books open as a blank frame. The parent
...
// serves the same book paths and swap it in:
//
//   node scripts/rehost.mjs --list
//       show every host in books.json with a count
//
...
// books in the player afterwards, since a host that answers 200 on the root
// does not prove it serves the same book paths.

import { readFileSync, writeFileSync } from 'node:fs'
import { promises as dns } from 'node:dns'

const FILE = new URL('../public/books.json', import.meta.url)
...
// though the book itself is fine.
...
if (has('list') || process.argv.length <= 2) {
  console.log(`${books.length} books across ${hosts.size} hosts\n`)
...
console.log(`${changed} of ${books.length} books would move from ${from} to ${to}\n`)
...
if (has('write')) {
  ...
  console.log(`\nwrote public/books.json. Now spot check a few books in the player.`)
} else {
